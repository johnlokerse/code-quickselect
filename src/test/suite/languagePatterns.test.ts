...
import { getBicepPatterns, getCSharpPatterns, getPowerShellPatterns } from '../../providers/languagePatterns';

suite('languagePatterns', () => {
  test('Bicep patterns match resource declarations', () => {
    const text = "resource myRes 'Microsoft.Storage/storageAccounts@2021-09-01' = {\n}";
...
public class SampleClass
{
    ...
    const methodMatch = patterns.some((pattern) =>
      new RegExp(pattern.source, pattern.flags).test('public async Task<string> GetValueAsync(int id)')
    );

    assert.ok(classMatch);
    assert.ok(enumMatch);
    assert.ok(methodMatch);
    assert.ok(patterns.some((pattern) => new RegExp(pattern.source, pattern.flags).test(text)));
  });
});
